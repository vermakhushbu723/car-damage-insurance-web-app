    ...
    const handlePhotoPointClick = (point) => {
        ...
        if (point.id === 'video') {
            navigate(ROUTES.WALK_AROUND_VIDEO, {
                state: { returnTo: '/preinspection-weblink/photo-capture-selection' },
            });
            return;
        }
        ...
        navigate(`/preinspection-weblink/camera-capture/${point.id}`, {
            state: {
                returnTo: '/preinspection-weblink/photo-capture-selection',
                cameraEnabled: true,
            },
        });
    };
