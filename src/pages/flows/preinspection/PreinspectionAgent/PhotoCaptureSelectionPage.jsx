    ...
    const handlePhotoPointClick = (point) => {
        ...
        if (point.id === 'video') {
            navigate(ROUTES.WALK_AROUND_VIDEO, {
                state: { returnTo: '/preinspection-agent/photo-capture-selection' },
            });
            return;
        }
        ...
        navigate(`/preinspection-agent/camera-capture/${point.id}`, {
            state: {
                returnTo: '/preinspection-agent/photo-capture-selection',
                cameraEnabled: true,
            },
        });
    };
