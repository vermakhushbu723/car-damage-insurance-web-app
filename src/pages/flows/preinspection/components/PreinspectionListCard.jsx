...
// Preinspection variant of the dashboard list card. Same layout as the
// claim ClaimListCard, but preinspection-specific field labels:
//   Claim Number  → PI Ref. Number
//   Insured Name  → Owner Name
// The demo data fields (claimNumber / insuredName) are reused as the
// underlying values until preinspection gets its own dataset.
const PreinspectionListCard = ({ claim, onViewDetails }) => {
    ...
    return (
        <div className="mb-3 rounded-lg px-3 py-2" style={{ background: '#DAF0FE' }}>
            ...
            {[
                ...
                { icon: claimNumberIcon, label: 'PI Ref. Number', value: claim.claimNumber },
                { icon: registrationNumberIcon, label: 'Registration Number', value: claim.registrationNumber },
                { icon: insuredNameIcon, label: 'Owner Name', value: claim.insuredName },
            ].map((row, i) => (
...
export default PreinspectionListCard;
