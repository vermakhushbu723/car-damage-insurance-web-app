...
const FLAPage = () => (
    <main style={{ padding: '20px 24px', fontFamily: 'Instrument Sans, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
            <div>
                <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#1454D1' }}>FLA (First Loss Approval)</h1>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
            <MetricCard title="Total Approvals" value="03" iconBg="#EFF6FF" iconBorderColor="#BFDBFE" icon={<ClipIcon />} sparkVariant="blue" />
            <MetricCard title="Pending" value="01" iconBg="#FFFBEB" iconBorderColor="#FDE68A" icon={<ClockIcon />} sparkVariant="orange" />
            <MetricCard title="Approved" value="01" iconBg="#ECFDF5" iconBorderColor="#A7F3D0" icon={<CheckIcon />} sparkVariant="green" />
            <MetricCard title="Total Approved" value="₹15.0L" iconBg="#F5F3FF" iconBorderColor="#DDD6FE" icon={<BoxIcon />} sparkVariant="purple" />
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                    <tr style={{ background: '#F9FAFB' }}>
                        {['FLA Details', 'Claim Info', 'Approval Amount', 'Approved By', 'Status', 'Action'].map(h => (
                    ...
                    {rows.map((row, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #F3F4F6' }}>
                            <td style={{ padding: '14px 16px' }}>
                                <div style={{ fontWeight: 700, color: '#111827' }}>{row.flaId}</div>
                            ...
                            <td style={{ padding: '14px 16px' }}>
                                ...
                                <div style={{ color: '#6B7280', fontSize: 12 }}>{row.claimNo}</div>
                            </td>
                            <td style={{ padding: '14px 16px' }}>
                                <div style={{ color: '#111827' }}>{row.approvalAmount}</div>
                                <div style={{ color: '#6B7280', fontSize: 12 }}>{row.approvalDate}</div>
                            </td>
                            <td style={{ padding: '14px 16px', color: '#374151' }}>{row.approvedBy}</td>
